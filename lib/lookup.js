...
module.exports = function lookup (name) {
  ...
  if (!task) {
    throw new Error('Invalid task `' + name + '`. Register `' + name + '` before building.');
  }
  var flow = utils.bach[task.options.flow || 'series'];
  var deps = task.deps.map(lookup.bind(this));
  return flow.apply(utils.bach, deps.concat([task.run.bind(task)]));
};
