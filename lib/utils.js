...
/**
 * Determine if the args array contains a prompt message.
 *
 * ```js
 * console.log(isPrompt(['Is this a prompt?']));
 * //=> true
 * console.log(isPrompt(['foo']));
 * //=> false
 * ```
 * @param  {Array} `args` Array of arguments
 * @return {Boolean} true if `args` has a prompt message
 */

utils.isPrompt = function(args) {
  return args.length
