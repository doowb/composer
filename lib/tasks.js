...
module.exports = Emitter => {
  ...
  class Tasks extends Emitter {
    ...
    task(name, ...tasks) {
      ...
      const deps = tasks.filter(val => val !== options);
      const callback = typeof last(deps) === 'function' ? deps.pop() : cb => cb();
    ...
    async build(...args) {
      ...
      args = flatten(args);
      const cb = typeof last(args) === 'function' ? args.pop() : null;
    ...
    disableInspect() {
      const fn = Task.prototype.inspect;
      delete Task.prototype.inspect;
      return () => {
        Task.prototype.inspect = fn;
      };
    }
