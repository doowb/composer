...
module.exports = Emitter => {
  ...
  class Tasks extends Emitter {
    ...
    task(name, ...tasks) {
      ...
      const options = tasks.find(val => utils.typeOf(val) === 'object' && !val.isTask) || {};
      const deps = tasks.filter(val => val && val !== options);
      const callback = typeof utils.last(deps) === 'function' ? deps.pop() : noop;
    ...
    setTask(name, options = {}, deps = [], callback) {
      ...
      task.on('starting', emit);
      task.on('finished', emit);
    ...
    build(...args) {
      ...
      let { options, tasks } = createOptions(this, ...args);
      ...
      return resolveBuild(this, promise, cb);
    }
    ...
    series(...args) {
      ...
      const compose = this.iterator(async(tasks, options, resolve) => {
        for (const ele of tasks) {
          ...
          task.on('finished', () => stack.delete(task));
          task.on('starting', () => stack.add(task));
...
function resolveBuild(app, promise, cb) {
  if (typeof cb === 'function') {
    promise.then(() => cb(null, app.taskState)).catch(cb);
    return;
  }
  return promise.then(() => app.taskState);
}

function createOptions(app, ...rest) {
  const args = utils.flatten(rest);
  const config = args.find(val => utils.typeOf(val) === 'object' && !val.isTask) || {};
