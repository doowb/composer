...
module.exports = (Emitter = require('./events')) => {
  ...
  class Tasks extends Emitter {
    constructor(options = {}) {
      ...
      // super(Emitter.name !== 'Emitter' && Emitter.name !== 'EventEmitter' ? options : null);
    ...
    expandTasks(...args) {
      ...
      for (const task of vals) {
        ...
        throw new TypeError(msg + utils.typeOf(task));
      }
      return tasks;
    }
    ...
    async build(...args) {
      ...
      let { options, tasks } = utils.createOptions(this, ...args);
    ...
    iterator(fn) {
      return (...args) => {
        const { options, tasks } = utils.createOptions(this, ...args);
