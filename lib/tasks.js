...
const factory = (Emitter = Events) => {
  ...
  class Tasks extends Emitter {
    constructor(options = {}) {
      ...
      this.tasks = new Map();
      this.taskId = 0;
      if (typeof this.off === 'undefined' && typeof this.removeListener !== 'undefined') {
    ...
    series(...args) {
      let stack = new Set();
      let compose = this.iterator(async(tasks, options, resolve) => {
    ...
    parallel(...args) {
      let stack = new Set();
      let compose = this.iterator((tasks, options, resolve) => {
        let pending = [];
