    ...
    expandTasks(...args) {
      ...
      for (let task of args) {
        if (typeof task === 'function') {
          const name = (this.tasks.has('default') || args.indexOf('default') > -1)
            ? ('task-' + this.taskId++)
            : 'default';
...
function resolve(app, promise, cb) {
  if (typeof cb === 'function') {
    promise.then(() => cb(null, app.taskPayload)).catch(cb);
    return;
  }
  return promise.then(() => app.taskPayload);
}
