...
const factory = (Emitter = require('./events')) => {
  class Generator extends Tasks(Emitter) {
    ...
    inspect() {
      if (typeof this.options.inspectFn === 'function') return this.options.inspectFn(this);
    ...
    async generate(...tasks) {
      const options = tasks.find(val => utils.typeOf(val) === 'object') || {};
      const opts = Object.assign({}, this.options, options);
      const rest = tasks.filter(val => val && val !== options);
