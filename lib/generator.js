...
const factory = (Emitter = Events) => {
  ...
  class Generator extends Tasks.create(Emitter) {
    ...
    async generate(...args) {
      let parsed = parse(this, ...args);
      let { tasks, options, callback } = parsed;

      const generator = name => this.hasGenerator(name)
        ? this.getGenerator(name, options)
        : this;

      const promise = new Promise(async(resolve, reject) => {
        if (options.parallel === true) {
          let pending = [];
          for (const ele of tasks) pending.push(generator(ele.name).build(ele.tasks));
    ...
    formatError(name, type = 'generator') {
      ...
      let suffix = this !== this.base ? `generator "${key}"` : 'the base generator';
    ...
    disableInspect() {
      const inspect = this[util.inspect.custom];
