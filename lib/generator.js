...
const factory = (Emitter = Events) => {
  ...
  class Generator extends Tasks(Emitter) {
    ...
    toGenerator(name, config, fn) {
      ...
      utils.define(generator, 'name', alias);
      utils.define(generator, 'parent', this);
      utils.define(generator, 'instance', null);
      utils.define(generator, 'called', 0);
    ...
    formatError(name, type = 'generator') {
      ...
      let message = `${type} "${name}" is not registered on `;
      let suffix = this.namespaces ? `generator "${key}"` : 'the root generator';
    ...
    disableInspect() {
      ...
      this.inspect = null;
      return () => {
        utils.define(this, 'inspect', inspect);
      };
    }
    ...
    /**
     * Static getter to simplify getting the `Task` class.
     *
     * ```js
...
/**
 * Returns true if `name` is registered on the "default"
 * task or generator.
 */
...
function isDefault(app, name) {
  ...
  const key = [].concat(name)[0].split('.').shift();
